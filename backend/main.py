...
def db_get_user(user_id: str) -> dict:
    ...
    with get_db() as conn:
        ...
        name_val = row["name"]
        if name_val and name_val.lower() in {"feeling", "user", "anonymous"}:
            name_val = None
...
def extract_context_from_message(text: str, ctx: dict) -> dict:
    ...
    # Name extraction
    # 1. Explicit name patterns (always take priority and can update name)
    explicit_nm = re.search(
        r"(?:my name is|mera naam hai|call me|naam hai mera)\s+([A-Za-z]+)",
        text, re.IGNORECASE
    )
    if explicit_nm:
        extracted = explicit_nm.group(1).strip().capitalize()
        if extracted.lower() not in {"a", "the", "user", "nobody", "anonymous", "feeling"}:
            ctx["name"] = extracted
    elif not ctx.get("name") or ctx.get("name").lower() in {"feeling", "user", "anonymous"}:
        # 2. Ambiguous patterns like "I am X" / "I'm X"
        ambiguous_nm = re.search(
            r"(?:i am|i'm)\s+([A-Za-z]+)",
            text, re.IGNORECASE
        )
        if ambiguous_nm:
            word = ambiguous_nm.group(1).strip()
            non_name_words = {
                "feeling", "stressed", "anxious", "sad", "happy", "depressed",
                "tired", "fine", "good", "bad", "ok", "okay", "so", "really",
                "very", "just", "doing", "going", "getting", "not", "always",
                "never", "here", "there", "home", "sorry", "afraid", "scared",
                "alone", "lonely", "angry", "furious", "confused", "lost",
                "worried", "hopeless", "hurt", "sick", "bored", "busy", "a", "an", "the"
            }
            if word.lower() not in non_name_words and len(word) > 1:
                ctx["name"] = word.capitalize()
...
# Local MuRIL model loading and classification is managed by MindMateSentimentAnalyzer
 
 
def detect_emotion(text: str, previous_context: str = None) -> Tuple[str, float, str, str]:
    try:
        result = mindmate_analyzer.predict_emotion(text, previous_context=previous_context)
...
def chat(req: ChatReq):
    ...
    # Fetch recent context for multi-turn emotion detection
    history = db_get_messages(req.user_id, limit=4)
    prev_user_msgs = [m["content"] for m in history if m.get("role") == "user"]
    prev_ctx = " ".join(prev_user_msgs[-2:]) if prev_user_msgs else None

    reply = groq_chat(req.user_id, req.message, persona_hint=req.persona_hint)
    el, ec, ee, en = detect_emotion(req.message, previous_context=prev_ctx)
